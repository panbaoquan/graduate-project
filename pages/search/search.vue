	...
	<view class="content">
		...
		<view class="search-keyword" >
			<scroll-view class="keyword-list-box" v-show="isShowKeywordList" scroll-y>
				...
				<Store page="detail" v-for="(item, index) in Shops" :index="item.id" :shop="item" />
			</scroll-view>
			<scroll-view class="keyword-box" v-show="!isShowKeywordList" scroll-y>
				...
				<view class="keyword-block">
					<view class="keyword-list-header">
						<view>热门搜索</view>
						<view>
							<image @tap="hotToggle" :src="require('@/static/search/attention'+forbid+'.png')"></image>
						</view>
					</view>
...
<script>
	...
	import mSearch from '@/pages/search/components/mehaotian-search-revision.vue';
	...
	export default {
		...
		methods: {
			...
			loadHotKeyword() {
				...
				this.hotKeywordList = ['盖浇饭', '麻辣烫', '华莱士', '奶茶', '水饺', '炒饭', '粥', '包子', '黄焖鸡米饭'];
			}, 
			//监听输入
			inputChange(event) {
				//兼容引入组件时传入参数情况
				...
				// 	}
				// });
				apiShop.searchRestaurant(keyword).then(res=>{
...
<style>
	view{display:block;}
	.search-box {width:95%;background-color:rgb(242,242,242);padding:15upx 2.5%;display:flex;justify-content:flex-end;position:sticky;top: 0;}
